...
def tv_loss(name, input_layer, axis=1):
    ...
    with tf.get_default_graph().name_scope(name):
        tv_losses = []
        # tf.unstack() split the input_layer into arrays
        # axis=1 means that the n feature maps are split.
        unpacked = tf.unstack(input_layer, axis=axis)
        for i in range(len(unpacked)):
            tv_losses.append(tf.reduce_sum(
                tf.image.total_variation(unpacked[i])
            ))
        # tf.add_n() adds n elements in a list
...
def gramian(activations, normalize_method=None):
    ...
    similarity, we vectorized and compute the entire gramian in a single matrix
    multiplication
    """
