...
def load_image(path, size=(200, 200)):
    ...
    load image (skimage reads as RGB HWC [0, 255] uint8, convert to RGB if gray
    VGG accepts BGR CHW [0, 255] float32
    :param path: path of the image.
    :param size: the image is resized to size.
    :return: resized image [0, 1]
    """
...
def get_immediate_subfiles(a_dir):
    """
    Get sorted files in a dir.
    :param a_dir:
    :return: type: list, filenames in the dir.
    """
...
def load_images(path, size=(40, 200, 200)):
    ...
    :param size: first parameter: amount of images; second and third parameters: cropped size
...
def get_bounds(images, im_size, frame_count):
    """
    Helper function to get optimization bounds from source image.
    Taken from Gatys's DeepTextures repo.
    :param images: a list of images
    :param im_size: image size for the generated image.
    :param frame_count:
    :return: list of bounds on each pixel for the optimization
    """
